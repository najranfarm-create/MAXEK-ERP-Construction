...
def list_revised_estimates(db, project_id: int | None = None) -> list[dict[str, Any]]:
    sql = (
        "SELECT e.*, p.project_code, p.project_name FROM revised_estimates e "
...
def load_boq_lines_for_project(db, project_id: int) -> list[dict[str, Any]]:
...
def save_revised_estimate(db, form, username: str) -> int:
    ...
    if not lines:
        raise ValueError("No BOQ lines to save.")
    ...
    if estimate_id_raw:
        ...
        db.execute(
            "UPDATE revised_estimates SET project_id=?, revision_date=?, title=?, remarks=?, "
        ...
        cur = db.execute(
            ...
            "project_id, revision_no, revision_date, title, remarks, status, "
    ...
    for line in lines:
        db.execute(
            ...
            "estimate_id, boq_item_id, item_code, item_description, unit, "
