...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function showDataEntryPanel(panel) {
    ...
    if (layout) {
      ...
      layout.classList.add('module-layout--entry-form-open');
      const listId = findEntryListId(panel);
      if (listId) {
        const listEl = document.getElementById(listId);
        if (listEl) listEl.setAttribute('hidden', '');
      }
      const hideIds = (panel.getAttribute('data-entry-hide-on-open') || '').split(',').map(function (s) {
        return s.trim();
      }).filter(Boolean);
      ...
      layout.querySelectorAll('[data-hide-on-entry-open]').forEach(function (el) {
        el.setAttribute('hidden', '');
      });
      layout.querySelectorAll('.erp-module-table-panel, .erp-table-panel, [data-erp-table]').forEach(function (el) {
        if (el.id && el.id !== listId && !hideIds.includes(el.id)) {
  ...
  function getPageNavContext() {
    const body = document.body;
    return {
      listUrl: body.getAttribute('data-page-list-url') || '',
      parentUrl: body.getAttribute('data-page-parent-url') || '',
      crudMode: body.getAttribute('data-page-crud-mode') || 'list',
    };
  }

  function stripCrudParamsFromHref(href) {
    if (!href) return '';
    try {
      const url = new URL(href, window.location.origin);
      ['view', 'edit', 'new', 'view_document', 'edit_document'].forEach(function (key) {
  ...
  function resolveEntryListHref(panel) {
    const ctx = getPageNavContext();
    if (ctx.listUrl) return ctx.listUrl;
    const backHref = panel && (panel.getAttribute('data-entry-back-href') || '');
    if (backHref) return stripCrudParamsFromHref(backHref);
    return stripCrudParamsFromHref(window.location.href);
  }

  function navigateToEntryList(panel) {
    const target = resolveEntryListHref(panel);
    const hasOpenPanel = panel && !panel.hasAttribute('hidden');
    const onSameModule = target && target.split('#')[0] === (window.location.pathname + window.location.search);
    if (hasOpenPanel && onSameModule && !getPageNavContext().crudMode.match(/^(view|edit|create)$/)) {
      hideDataEntryPanel(panel, { scrollToList: true });
      return;
    }
    window.location.assign(target.split('#')[0] + (target.indexOf('#') >= 0 ? target.slice(target.indexOf('#')) : ''));
  }
  ...
  function hideDataEntryPanel(panel, options) {
    ...
    if (layout) {
      ...
      layout.classList.remove('module-layout--entry-form-open');
      const hideIds = (panel.getAttribute('data-entry-hide-on-open') || '').split(',').map(function (s) {
        return s.trim();
      }).filter(Boolean);
      hideIds.forEach(function (id) {
    ...
    const stripped = stripCrudParamsFromHref(window.location.href);
    const base = stripped.split('#')[0];
    const hash = listId ? '#' + listId : '';
  ...
  document.addEventListener('click', function (event) {
    ...
    if (backBtn) {
      ...
      if (!panel) return;
      const ctx = getPageNavContext();
      if (ctx.crudMode === 'view' || ctx.crudMode === 'edit' || ctx.crudMode === 'create') {
        window.location.assign(resolveEntryListHref(panel).split('#')[0]);
        return;
      }
      navigateToEntryList(panel);
    }
  });
