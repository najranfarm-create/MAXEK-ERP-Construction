(function () {
  ...
  function renderBoqCheckboxList(form) {
    ...
    panel.hidden = !state.multiBoqPanelOpen || !state.boqItems.length;
    updateSelectedBoqCount(form);
  }
  ...
  function initMultiBoqHandlers(form) {
    ...
    var toggleBtn = form.querySelector("[data-dpr-toggle-multi-boq]");
    var multiPanel = form.querySelector("[data-dpr-multi-boq-panel]");
    if (toggleBtn && multiPanel) {
      toggleBtn.addEventListener("click", function () {
        state.multiBoqPanelOpen = !state.multiBoqPanelOpen;
        multiPanel.hidden = !state.multiBoqPanelOpen || !state.boqItems.length;
        if (!multiPanel.hidden) {
  ...
  function selectBoqItem(form, itemId) {
    ...
    if (descSel && descSel.tagName === "SELECT") descSel.value = String(item.id);
    if (boqItemHidden && boqItemHidden.tagName !== "SELECT") boqItemHidden.value = String(item.id);
    if (numHidden) numHidden.value = item.boq_number || "";
    if (descHidden) descHidden.value = item.item_description || "";
    ...
    if (!isEditMode(form)) {
      state.selectedBoqIds = [String(item.id)];
      renderSelectedBoqSummary(form);
    } else {
  ...
  function showMeasurementPanel(form, unit) {
    ...
    var hint = form.querySelector("[data-dpr-boq-pick-hint]");
  ...
  function buildManpowerPayload(form) {
    ...
    form.querySelectorAll("[data-dpr-manpower-row]").forEach(function (row) {
      ...
      var headcount = parseNum((row.querySelector("[data-mp-headcount]") || {}).value || 1) || 1;
      var hoursEach = parseNum((row.querySelector("[data-mp-hours]") || {}).value);
      var remarks = (row.querySelector("[data-mp-remarks]") || {}).value || "";
      if (headcount > 1 && remarks.indexOf("Headcount:") !== 0) {
        remarks = "Headcount: " + headcount + (remarks ? " — " + remarks : "");
      }
      ...
      rows.push({
        ...
        trade_name: (row.querySelector("[data-mp-trade]") || {}).value || "",
        hours_worked: hoursEach * headcount,
        remarks: remarks,
      });
    });
    return rows;
  ...
  function loadBoqItems(form, projectId, done) {
    ...
    if (descSel) descSel.disabled = true;
    fetch("/api/projects/" + projectId + "/boq-items")
      .then(function (r) {
        if (!r.ok) throw new Error("Failed to load BOQ items");
        return r.json();
      })
      .then(function (items) {
        state.boqItems = items || [];
        fillBoqDropdowns(form);
        if (!state.boqItems.length && numSel) {
          numSel.disabled = true;
          if (descSel) descSel.disabled = true;
          window.alert("No BOQ items found for this project. Create BOQ in BOQ Management first.");
        }
        var continueBoq = form.getAttribute("data-continue-boq");
      ...
      .catch(function () {
        fillBoqDropdowns(form);
        showMeasurementPanel(form, "");
        window.alert("Could not load BOQ items for this project. Check your connection and try again.");
        if (typeof done === "function") done();
      });
  }
  ...
  function initDprOpenSaved() {
    var records = window.DPR_SAVED_RECORDS || [];
    var dateInp = document.getElementById("dpr_open_date");
    var projectSel = document.getElementById("dpr_open_project");
    var recordSel = document.getElementById("dpr_open_record");
    var viewBtn = document.getElementById("dpr_open_view");
    var editBtn = document.getElementById("dpr_open_edit");
    if (!recordSel || !records.length) return;

    function filteredRecords() {
      var dateVal = dateInp ? dateInp.value : "";
      var projectVal = projectSel ? projectSel.value : "";
      return records.filter(function (row) {
        if (dateVal && (row.report_date || "") !== dateVal) return false;
        if (projectVal && String(row.project_id) !== String(projectVal)) return false;
        return true;
      });
    }
    ...
    function refreshRecordOptions() {
      ...
      var current = recordSel.value;
      recordSel.innerHTML = '<option value="">— Select saved DPR —</option>';
      list.forEach(function (row) {
        var opt = document.createElement("option");
        opt.value = String(row.id);
        opt.textContent = "#" + row.id + " · " + (row.report_date || "—") + " · "
          + (row.boq_number || "—") + " — " + ((row.boq_description || "").slice(0, 40));
        opt.dataset.canEdit = row.can_edit ? "1" : "0";
        recordSel.appendChild(opt);
      });
      recordSel.disabled = !list.length;
      if (current && list.some(function (r) { return String(r.id) === current; })) {
    ...
    function updateOpenButtons() {
      var has = !!recordSel.value;
      if (viewBtn) viewBtn.disabled = !has;
      if (editBtn) {
        var opt = recordSel.selectedOptions[0];
        editBtn.disabled = !has || !(opt && opt.dataset.canEdit === "1");
      }
    }

    if (dateInp) dateInp.addEventListener("change", refreshRecordOptions);
    if (projectSel) projectSel.addEventListener("change", refreshRecordOptions);
    recordSel.addEventListener("change", updateOpenButtons);

    if (viewBtn) {
      viewBtn.addEventListener("click", function () {
        if (!recordSel.value) return;
        window.location.href = "/dpr-entry?view=" + encodeURIComponent(recordSel.value);
      });
    }
    if (editBtn) {
      editBtn.addEventListener("click", function () {
        if (!recordSel.value) return;
        window.location.href = "/dpr-entry?edit=" + encodeURIComponent(recordSel.value) + "#dpr-form";
      });
    }
  ...
  function initDprForm() {
    ...
    form.addEventListener("submit", function (e) {
      if (!isEditMode(form) && !state.selectedBoqIds.length) {
        var descSel = form.querySelector("select[data-dpr-boq-description]");
        if (descSel && descSel.value) {
          selectBoqItem(form, descSel.value);
        }
      }
      if (!isEditMode(form) && !state.selectedBoqIds.length) {
        e.preventDefault();
        window.alert("Select a BOQ item (or use Bulk BOQ Entry) before saving.");
        return;
      }
