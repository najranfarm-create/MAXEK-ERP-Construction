(function () {
  ...
  if (backBtn) {
    backBtn.addEventListener('click', function () {
      ...
      var crudMode = body.getAttribute('data-page-crud-mode') || 'list';
      var listUrl = body.getAttribute('data-page-list-url') || '';
      var parentUrl = body.getAttribute('data-page-parent-url') || '';
      var openPanel = document.querySelector(
        '.erp-module-form-panel[data-entry-panel]:not([hidden]), '
        + '.erp-data-entry-panel[data-entry-panel]:not([hidden]), '
        + '.erp-form-card[data-entry-panel]:not([hidden])'
      );

      if (openPanel && window.MaxekDataEntryPanels) {
        window.MaxekDataEntryPanels.hide(openPanel, { scrollToList: true });
        return;
      }

      if (crudMode !== 'list' && listUrl) {
        window.location.href = listUrl;
        return;
      }

      var fallback = backBtn.getAttribute('data-fallback-url') || parentUrl;
      if (fallback) {
