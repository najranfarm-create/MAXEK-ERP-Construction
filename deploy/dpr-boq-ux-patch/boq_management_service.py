...
def parse_boq_items_from_form(form) -> list[dict[str, Any]]:
    ...
    for idx in range(count):
        ...
        unit_manual = lists["unit_manual"][idx] if idx < len(lists["unit_manual"]) else ""
        unit_val = (unit_manual or lists["unit"][idx] if idx < len(lists["unit"]) else "").strip() or "Nos"
        items.append(
            {
                "id": item_id,
                "line_no": idx + 1,
...
def list_boqs_for_project(db, project_id: int) -> list[dict[str, Any]]:
    """Current BOQ headers for a project (BOQ number dropdown)."""
...
def list_boq_description_history(
    db, project_id: int | None = None, limit: int = 500
) -> list[dict[str, Any]]:
    """Distinct BOQ descriptions for combobox — library + project / global history."""
    ensure_boq_management_schema(db)
    try:
        ...
        library_rows = list_standard_boq_library(db, active_only=True)
    except Exception:
        library_rows = []

    index: dict[str, dict[str, Any]] = {}

    def _add(desc: str, unit: str = "", rate: float = 0, item_code: str = "", source: str = ""):
        ...
        payload = {
            ...
            "unit": (unit or "").strip() or "Nos",
            "rate": _float(rate),
            "item_code": (item_code or "").strip(),
            "source": source,
        }
        if not existing or (payload["rate"] and not existing.get("rate")):
            index[key] = payload
    ...
    for row in library_rows:
        _add(
            ...
            row.get("unit") or "",
            row.get("standard_rate") or 0,
            row.get("boq_code") or row.get("item_number") or "",
            "library",
        )

    if project_id:
        for item in get_boq_items_for_project(db, project_id):
            _add(
                item.get("item_description") or item.get("description") or "",
                item.get("unit") or "",
                item.get("rate") or 0,
                item.get("item_code") or item.get("item_number") or "",
                "project",
            )
    else:
        rows = db.execute(
            """
            SELECT DISTINCT item_description, unit, rate, item_code, item_number
            FROM boq_items
            WHERE COALESCE(is_deleted,0)=0 AND TRIM(COALESCE(item_description,''))!=''
            ORDER BY id DESC
            LIMIT ?
            """,
            (limit,),
    ...
    results.sort(key=lambda x: (x.get("description") or "").lower())
