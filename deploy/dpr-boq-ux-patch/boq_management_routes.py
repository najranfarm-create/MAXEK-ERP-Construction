...
def register_boq_management_routes(
    ...
    *,
    ...
    @app.route("/api/boq-management/project/<int:project_id>/boqs", methods=["GET"])
    @login_required
    def boq_management_project_boqs(project_id: int):
        denied = _require("view")
        if denied:
            return denied
        from boq_management_service import list_boqs_for_project

        db = get_db()
        return jsonify(
            {
                "project_id": project_id,
                "boqs": list_boqs_for_project(db, project_id),
            }
        )

    @app.route("/api/boq-management/description-history", methods=["GET"])
    @login_required
    def boq_management_description_history():
        denied = _require("view")
        if denied:
            return denied
        from boq_management_service import list_boq_description_history
        ...
        return jsonify(
            {
                "project_id": project_id,
                "items": list_boq_description_history(db, project_id=project_id),
            }
        )
